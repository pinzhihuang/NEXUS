...
def update_or_create_news_document(school: dict[str, str], reports_data: list, week_start_date: datetime.date, week_end_date: datetime.date, is_email: bool = False) -> str | None:
    """
    ...
    try:
        ...
        if is_email:
            document_title = f"{school['school_name']}: Project NEXUS: Breaking News Digest ({week_start_date.strftime('%m/%d/%Y')})"
        else:
        ...
        if is_email:
            main_header_text = f"紧急新闻摘要 (Breaking News Digest): {week_start_date.strftime('%Y年%m月%d日')}\n\n"
        else:
...
if __name__ == '__main__':
    ...
    if not os.path.exists(config.OAUTH_CREDENTIALS_FILE):
        ...
        doc_link = update_or_create_news_document(school, sample_reports, test_week_start_date, test_week_end_date, is_email=False)
